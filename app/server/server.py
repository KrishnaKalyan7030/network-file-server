...
METADATA_FILE = os.path.join(BASE_DIR, '../../database/file_metadata.json')
USERS_FILE = os.path.join(BASE_DIR, '../../database/users.json')
SHARED_METADATA_FILE = os.path.join(BASE_DIR, '../../database/shared_metadata.json')
...
def handle_client(client_socket, client_add):
    ...
    # Default role — will be set properly after successful login
    user_role = 'user'
    ...
    try:
        ...
        if command == 'REGISTER':
            ...
            if (
                ...
                log_activity(f"{username} logged in successfully")
        ...
        while True:
            ...
            if operation == 'UPLOAD':
                ...
                if os.path.exists(filepath):
                    client_socket.send("FILE_ALREADY_EXISTS".encode())
                    continue
                ...
                with open(filepath, 'wb') as file:
                    ...
                    while received_size < filesize:
                        ...
                        data = client_socket.recv(1024 if remaining > 1024 else remaining)
                        if not data:
                            break
                        file.write(data)
                ...
                filepath = os.path.join(UPLOAD_FOLDER, owner, filename)

                if not os.path.exists(filepath):
                    client_socket.send("FILE_NOT_FOUND".encode())
                    continue

                shared_users = shared_metadata.get(filename, [])
                ...
                if (
                    ...
                    log_activity(f"{username} unauthorized download attempt on {filename}")
                    continue
                ...
                if len(parts) < 3:
                    client_socket.send("Invalid RENAME command".encode())
                    continue

                old_filename = parts[1]
                new_filename = parts[2]

                owner = file_metadata.get(old_filename)

                if not owner:
                    client_socket.send("File not found".encode())
                    continue

                if owner != username and user_role != 'admin':
                    client_socket.send("Access Denied".encode())
                    continue

                old_filepath = os.path.join(UPLOAD_FOLDER, owner, old_filename)
                new_filepath = os.path.join(UPLOAD_FOLDER, owner, new_filename)
                ...
                if os.path.exists(new_filepath):
                    client_socket.send("New filename already exists".encode())
                    continue

                os.rename(old_filepath, new_filepath)

                file_metadata[new_filename] = file_metadata.pop(old_filename)

                if old_filename in shared_metadata:
                    shared_metadata[new_filename] = shared_metadata.pop(old_filename)

                with open(METADATA_FILE, 'w') as file:
                    json.dump(file_metadata, file, indent=4)

                with open(SHARED_METADATA_FILE, 'w') as file:
                    json.dump(shared_metadata, file, indent=4)

                client_socket.send("File renamed successfully".encode())
                log_activity(f"{username} renamed {old_filename} to {new_filename}")
                ...
                if user_role == 'admin':
                    ...
                    for owner_folder in os.listdir(UPLOAD_FOLDER):
                        owner_path = os.path.join(UPLOAD_FOLDER, owner_folder)
                        if os.path.isdir(owner_path):
                            for file in os.listdir(owner_path):
                                response += f"{file} -> {owner_folder}\n"
                    ...
                    for owner_folder in os.listdir(UPLOAD_FOLDER):
                        ...
                        if os.path.isdir(owner_path):
                            for file in os.listdir(owner_path):
                                ...
                                shared_users = shared_metadata.get(file, [])
                                if username == owner or username in shared_users:
                                    response += f"{file} -> {owner_folder}\n"
                ...
                log_activity(f"{username} viewed file list")

            elif operation == 'SHARE':

                if len(parts) < 3:
                    client_socket.send("Invalid SHARE command".encode())
                    continue
                ...
                if target_user not in users:
                    client_socket.send("Target user does not exist".encode())
                    continue
                ...
                if target_user in shared_metadata[filename]:
                    client_socket.send("File already shared".encode())
                    continue

                shared_metadata[filename].append(target_user)

                with open(SHARED_METADATA_FILE, 'w') as file:
                    json.dump(shared_metadata, file, indent=4)

                client_socket.send("File shared successfully".encode())
                log_activity(f"{username} shared {filename} with {target_user}")
                ...
                if os.path.exists(filepath):
                    ...
                    log_activity(f"{username} deleted {filename}")
                ...
                # Send in chunks so large log files don't get cut off
                encoded = history.encode()
                total = len(encoded)
                sent = 0
                while sent < total:
                    chunk = encoded[sent:sent + 4096]
                    client_socket.send(chunk)
                    sent += len(chunk)
        ...
        log_activity(f"Server error with {client_add}: {str(e)}")
...
while True:
    ...
    log_activity(f"New connection from {client_add}")
